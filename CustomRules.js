...
class Handlers
{
    ...
    */
    ...
    static function OnBeforeRequest(oSession: Session) {
        ...
        // php项目的url
		var ci_host = "http://localhost/gksvideo_in_pc/index.php";
		//我的关注,预记录所有的返回feeds记录
		if (oSession.uriContains("/rest/n/feed/myfollow")) {
		   oSession["ui-color"] = "green";
		}
        //当点击like的时候，进行下载
        if (oSession.uriContains("/rest/photo/like")) {
           oSession["ui-color"] = "pink";
        }

        //当进入个人用户的列表的时候，下载这个用户的所有视频记录
        if (oSession.uriContains("/rest/n/feed/profile2")) {
           oSession["ui-color"] = "orange";
        }

        // FiddlerObject.log('share photoid host:' + oSession.hostname);
        if (oSession.uriContains("/rest/n/share/sharePhoto")) {
            oSession["ui-color"] = "maroon";
            oSession.utilDecodeRequest();//消除保存的请求可能存在乱码的情况
            var requestString = oSession.GetRequestBodyAsString();
            var variable1 = 'photoId';
            var photoId = "";
            //FiddlerObject.log('like request photo:'+requestString);
            var vars = requestString.split('&');
            for (var i = 0; i < vars.length; i++) {
                var pair = vars[i].split('=');
                if (decodeURIComponent(pair[0]) == variable1) {
                    photoId =  decodeURIComponent(pair[1]);
                }
            }
            
            if(photoId != ""){
                FiddlerObject.log('share photoid:'+photoId + (photoId != ""));
                var _xhr2 = new ActiveXObject("Microsoft.XMLHTTP");
                var singleshare_url = ci_host + '/welcome/sshare/' + encodeURIComponent(photoId);
...
*/
    ...
    static function OnBeforeResponse(oSession: Session) {
        ...
        var ci_host = "http://localhost/gksvideo_in_pc/index.php";
        //我的关注,预记录所有的返回feeds记录
        if ((oSession.uriContains("/rest/n/feed/myfollow") || oSession.uriContains("/rest/n/feed/hot") || oSession.uriContains("/rest/n/feed/nearby")  || oSession.uriContains("/rest/n/feed/profile2"))) {
            ...
            if((responseJSON.JSONObject=='System.Collections.ArrayList' || responseJSON.JSONObject=='System.Collections.Hashtable')&&jsonString!='[]'&&jsonString!='{}'){
                ...
                var url = ci_host + '/welcome/myfollows';
                //不需要返回值所以写啦个空回调
        ...
        //当点击分享个人用户的时候，点击复制链接的时候用户所有视频和图片进行下载
        if (oSession.uriContains("/rest/n/share/shareProfile")) {
            oSession.utilDecodeRequest();//消除保存的请求可能存在乱码的情况
            var requestString = oSession.GetRequestBodyAsString();
            var variable1 = 'userId';
            var userId = 0;
            FiddlerObject.log('like requestString:'+requestString);
            var vars = requestString.split('&');
            for (var i = 0; i < vars.length; i++) {
                var pair = vars[i].split('=');
                if (decodeURIComponent(pair[0]) == variable1) {
            ...
            FiddlerObject.log('share userId：'+userId);
            if(userId > 0){
                var _xhr = new ActiveXObject('Microsoft.XMLHTTP');
                var url = ci_host + '/welcome/profileshare/'+userId;
                //不需要返回值所以写啦个空回调
                _xhr.onreadystatechange = function() {}
                _xhr.open('POST', url, true);
                 _xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded;charset=utf-8");
